...
export default function ReferralCode() {
  ...
  useEffect(() => {
    if (user && isAuthenticated) {
      setUrl(
        `${window.location.origin}${basePath}/referral/${user?.my_referal_code}`
      );
    }
  }, [user]);
  ...
  return (
    ...
    {({ open }) => (
      <div>
        <Disclosure.Button
          className="flex w-full justify-between text-left text-sm font-medium focus:outline-none bg-zinc-800 rounded-md p-3"
        >
      ...
      <div className="flex flex-col gap-2">
        ...
        <span className="text-base font-bold">{t(`Your Referral Code is :`)}{''} {user?.my_referal_code}</span>
      </div>
      <div className="">
        <span className="text-sm font-medium">
          {t('Share this code with your friends and earn 50 points each time a friend uses it to register for an abracadabra account.')}
        </span>
      </div>
      <button className="flex justify-between items-center w-full py-2" onClick={handleShare}>
        <span className="text-sm font-medium">{t('Click here to share your referral codes.')}</span>
        <img
      ...
      {modalShow && <ModalShare url={url} text={shareText} quote={shareText} onHide={() => setModalShow(false)} />}
    </div>
        </Disclosure.Panel>
      </div>
    )}
