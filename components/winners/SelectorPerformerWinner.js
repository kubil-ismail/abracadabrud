...
export default function SelectorPerformerWinner({ eventId }) {
  ...
  useEffect(() => {
    ...
    service
      .get(
        `${process.env.NEXT_PUBLIC_SITE_URL}/api/lucky-draws/${eventId}/selector-performer-winners?language=${locale}`
      )
      .then((res) => {
        setData(res?.data);
      })
