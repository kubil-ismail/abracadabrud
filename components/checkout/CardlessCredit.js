...
export default function CardlessCredit({ paymentMethods }) {
  const dispatch = useDispatch();
  const { t } = useTranslation();
  const { paymentMethodsId, isUsingCC } = useSelector((state) => state.payments);
  return (
    <div className="flex flex-col gap-5">
      <h3 className="text-base font-bold">{t('Akulaku PayLater')}</h3>
      <div className="flex flex-wrap gap-5 justify-center">
        {paymentMethods
          ?.filter((item) => item.payment_type === 'cardless_credit')
          .map((data) => (
            <button
              className={`${
